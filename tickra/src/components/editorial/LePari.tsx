    ...
    >
      <div className="ed-section px-8 md:px-16">
        ...
        <div className="mt-16 md:mt-24 grid grid-cols-12 gap-x-6">
          <p
            className="col-span-12 lg:col-span-10 ed-display text-white text-balance"
            style={{ fontSize: 'clamp(40px, 6.4vw, 96px)', lineHeight: 0.98 }}
          >
            {locale === 'fr' ? (
              <>
                Gratuit pour s’asseoir au fond.{' '}
                <span className="text-white/55">Quatorze euros par mois</span> pour entrer en classe.{' '}
                <span className="text-white/35">Cent quarante-neuf,</span>{' '}
                <span className="text-white/55">une fois,</span> pour ne plus en sortir.
              </>
            ) : (
              <>
                Free for the back row.{' '}
                <span className="text-white/55">Fourteen euros a month</span> to take the class.{' '}
                <span className="text-white/35">One hundred forty-nine,</span>{' '}
                <span className="text-white/55">once,</span> to stay for life.
              </>
            )}
          </p>
        </div>
        ...
        <div className="mt-20 md:mt-28 grid grid-cols-12 gap-x-6 gap-y-10 border-t border-white/[0.06] pt-12">
          ...
          <div className="col-span-12 flex justify-between items-baseline mt-12">
            ...
            <Link
              href={`/${locale}/pricing`}
              className="ed-tag text-white/85 hover:text-white transition-colors"
            >
...
function Foot({ index, label, value }: { index: string; label: string; value: string }) {
  return (
    <div className="col-span-6 md:col-span-3">
      <div className="ed-tag text-white/30 tabular-nums">{index}</div>
      <div className="mt-3 text-white text-[15px]">{label}</div>
      <div className="mt-1 text-white/45 text-[13.5px]">{value}</div>
    </div>
  );
}
