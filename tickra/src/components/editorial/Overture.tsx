...
export function Overture({ locale }: Props) {
  ...
  return (
    ...
    >
      ...
      <div aria-hidden className="pointer-events-none absolute top-0 bottom-0 left-[8.33%] w-px bg-white/[0.06]" />
      <div aria-hidden className="pointer-events-none absolute top-0 bottom-0 right-[8.33%] w-px bg-white/[0.06]" />
      ...
      <header className="absolute top-10 left-0 right-0 px-8 md:px-16 flex items-start justify-between">
        <span className="ed-tag text-white/55">{overline}</span>
        <span className="ed-tag text-white/40 tabular-nums">{issue}</span>
      </header>

      {/* Big phrase, bottom-left aligned. Anchored deliberately low. */}
      <h1
        id="overture"
        className="absolute bottom-[16vh] left-8 md:left-16 right-8 md:right-32 ed-display text-white"
      ...
      <footer className="absolute bottom-10 right-8 md:right-16">
        <span className="ed-tag text-white/45">— {signature}</span>
      </footer>

      {/* Folio number, vertical, far left bottom */}
      <span
        aria-hidden
        className="absolute bottom-10 left-8 md:left-16 ed-tag text-white/30 tabular-nums"
      >
        01 / 07
      </span>
    </section>
