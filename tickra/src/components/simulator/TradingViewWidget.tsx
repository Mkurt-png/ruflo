...
// Self-hosted klinecharts v9 with full drawing-tools toolbar — trendlines,
// rays, horizontal levels, rectangles, Fibonacci. No CDN, no ad-blocker
// risk. Per-symbol deterministic synthetic OHLC + live last-candle drift.
...
function buildSeries(symbol: string, count = 220): KLineData[] {
...
export function TradingViewWidget({ symbol }: { symbol: string }) {
  ...
  useEffect(() => {
    ...
    // Live tick: drift the last candle every 1.2s so the chart feels alive.
    const profile = SYMBOL_PROFILE[symbol] ?? SYMBOL_PROFILE['FX:EURUSD'];
  ...
  function pickTool(id: ToolId) {
    ...
    if (id === 'cursor') return;
    if (id === 'eraser') {
      // v9 removeOverlay() with no args removes all overlays.
