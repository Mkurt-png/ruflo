...
const COPY = {
  fr: {
    ...
    body: (losses: number, window: number, until: string) =>
      `Sur les ${window} dernières clôtures, ${losses} étaient des pertes et le cumul est négatif. La règle de nkNOWTrade est claire : le journal se referme jusqu’à ${until}. Ce n’est pas une punition, c’est une protection — celle qu’on n’arrive pas toujours à s’imposer seul.`,
    until: 'Lever à',
  ...
  en: {
    ...
    body: (losses: number, window: number, until: string) =>
      `Of your last ${window} closes, ${losses} were losses and the cumulative is negative. nkNOWTrade’s rule is plain: the journal stays shut until ${until}. Not a punishment — a protection you cannot always impose on yourself.`,
    until: 'Lifts at',
    override: 'Force open',
