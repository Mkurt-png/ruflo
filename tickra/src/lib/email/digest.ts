...
export function buildDigestEmail(
  ...
  if (locale === 'fr') {
    const subject = `Votre semaine nkNOWTrade — ${stats.xpThisWeek} XP, série ${stats.streak} j`;
    const text =
      `Bonjour${name},\n\n` +
      `Cette semaine sur nkNOWTrade :\n` +
    ...
    const html =
      ...
      `<p>Cette semaine sur nkNOWTrade :</p>` +
      `<ul>` +
  ...
  const subject = `Your nkNOWTrade week — ${stats.xpThisWeek} XP, ${stats.streak}-day streak`;
  const text =
    `Hi${name},\n\n` +
    `Your week on nkNOWTrade:\n` +
