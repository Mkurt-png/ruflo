...
function systemPrompt(locale: 'fr' | 'en') {
  if (locale === 'fr') {
    return `Tu es nkNOWTrade Coach, un coach de trading pédagogique pour des traders qui s'entraînent en démo sur nkNOWTrade.
...
Tu ne prédis JAMAIS le futur du marché. Tu n'utilises pas le mot "signal".`;
  ...
  return `You are nkNOWTrade Coach, an educational trading coach for traders practising on nkNOWTrade demo.
...
function buildUserPrompt(b: Body, locale: 'fr' | 'en') {
  const lines: string[] = [];
  if (locale === 'fr') {
    lines.push('Voici les détails d’un trade démo nkNOWTrade :');
    ...
    lines.push('Here are the details of a nkNOWTrade demo trade:');
    lines.push(`- Instrument: ${b.symbol ?? '?'}`);
