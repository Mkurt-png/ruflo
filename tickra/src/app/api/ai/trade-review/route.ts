...
function systemPrompt(locale: 'fr' | 'en') {
  if (locale === 'fr') {
    return `Tu es kNOWTrade Coach, un coach de trading pédagogique pour des traders qui s'entraînent en démo sur kNOWTrade.
...
Tu ne prédis JAMAIS le futur du marché. Tu n'utilises pas le mot "signal".`;
  ...
  return `You are kNOWTrade Coach, an educational trading coach for traders practising on kNOWTrade demo.
...
function buildUserPrompt(b: Body, locale: 'fr' | 'en') {
  const lines: string[] = [];
  if (locale === 'fr') {
    lines.push('Voici les détails d’un trade démo kNOWTrade :');
    ...
    lines.push('Here are the details of a kNOWTrade demo trade:');
    lines.push(`- Instrument: ${b.symbol ?? '?'}`);
